...
pparser = argparse.ArgumentParser(description = "Train Xgboost Model")
...
xgb_model.fit(train_feature_data, train_labels)

# Predict on the test set
predictions = xgb_model.predict(test_feature_data)
...
test_accuracy = xgb_model.score(test_feature_data, test_labels)
print(f"Test Accuracy: {test_accuracy * 100:.2f}%")
print()

# Calculate and print F1 Score
f1 = f1_score(test_labels, predictions, average='weighted')  # 'weighted' handles class imbalance
print(f"F1 Score: {f1*100:.2f}")

# Calculate and print AUROC
if len(np.unique(test_labels)) > 1:  # Ensure there are at least two classes in the test set
    probabilities = xgb_model.predict_proba(test_feature_data)[:, 1]  # Probability for the positive class
