...
# Train a CatBoost model (binary classification)
catboost_model = CatBoostClassifier(iterations=100, learning_rate=0.1, depth=6, random_state=42, verbose=0)
catboost_model.fit(train_feature_data, train_labels)

# Predict on the test set
predictions = catboost_model.predict(test_feature_data)
...
test_accuracy = catboost_model.score(test_feature_data, test_labels)
...
if len(np.unique(test_labels)) > 1:  # Ensure there are at least two classes in the test set
    probabilities = catboost_model.predict_proba(test_feature_data)[:, 1]  # Probability for the positive class
