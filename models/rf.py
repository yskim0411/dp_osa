...
rf_model.fit(train_feature_data, train_labels)

# Predict on the test set
predictions = rf_model.predict(test_feature_data)
...
test_accuracy = rf_model.score(test_feature_data, test_labels)
...
if len(np.unique(test_labels)) > 1:  # Ensure there are at least two classes in the test set
    probabilities = rf_model.predict_proba(test_feature_data)[:, 1]  # Probability for the positive class
    
    # Convert probabilities to binary predictions (1 if >= 0.5, else 0)
    predictions = (probabilities >= 0.5).astype(int)

    # Find indices where predictions differ from actual labels
