...
print("SVM Model")
# Train an SVM model (binary classification)
svm_model = SVC(kernel='linear', probability=True, random_state=42)  # probability=True 추가
svm_model.fit(train_feature_data, train_labels)

# Predict on the test set
predictions = svm_model.predict(test_feature_data)
...
test_accuracy = svm_model.score(test_feature_data, test_labels)
...
if len(np.unique(test_labels)) > 1:  # Ensure there are at least two classes in the test set
    probabilities = svm_model.predict_proba(test_feature_data)[:, 1]  # Probability for the positive class
