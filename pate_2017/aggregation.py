...
def noisy_max(logits, lap_scale, nb_labels, return_clean_votes=False):
  """
  ...
  if return_clean_votes:
    ...
    clean_votes = np.zeros((int(labels_shape[1]), nb_labels)) # 10 -> 2로 변경

  # Parse each sample
  for i in xrange(int(labels_shape[1])):
    # Count number of votes assigned to each class
    label_counts = np.bincount(labels[:, i], minlength=nb_labels) # minlength = 2로 바꿈
