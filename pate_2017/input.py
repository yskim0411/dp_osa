...
def ld_mnist(test_only=False):
  ...
  # MNIST data is originally in shape (num_samples, 28, 28), we need to reshape to (num_samples, 28, 28, 1)
  train_data = train_data.reshape(train_data.shape[0], 28, 28, 1).astype(np.float32)
  test_data = test_data.reshape(test_data.shape[0], 28, 28, 1).astype(np.float32)

  # Normalize the pixel values to the range [-0.5, 0.5]
  train_data = (train_data - 127.5) / 255.0
  test_data = (test_data - 127.5) / 255.0
  
  # Ensure that labels are in np.int32 format
  ...
  assert not np.isnan(train_data).any(), 'Training data contains NaN values!'
...
def ld_eumc_remosa(test_only=False):
  with open('/tabsyn/synthetic/pickle/tabsyn.pkl', 'rb') as f:
    train_data = pickle.load(f)
  with open('/tabsyn/synthetic/pickle/tabsyn_label.pkl', 'rb') as f:
    train_labels = pickle.load(f)

  # 테스트 데이터 로드 (실제 데이터 로드 함수로 대체 필요)
  with open('/data/remosa/4/pickle/test_feature.pkl', 'rb') as f:
    test_data = pickle.load(f)
  with open('/data/remosa/4/pickle/test_label.pkl', 'rb') as f:
    test_labels = pickle.load(f)
    
  with open('/data/remosa/4/pickle/query_feature.pkl', 'rb') as f:
    synthetic_data = pickle.load(f)
  with open('/data/remosa/4/pickle/query_label.pkl', 'rb') as f:
    synthetic_labels = pickle.load(f)
  
  
  train_labels = np.ravel(train_labels)
  test_labels = np.ravel(test_labels)
  synthetic_labels = np.ravel(synthetic_labels)
    
  if test_only:
    return test_data, test_labels, synthetic_data, synthetic_labels
  else:
    return train_data, train_labels, test_data, test_labels
  
  
def ld_eumc_posa(test_only=False):
  with open('/tabsyn/synthetic/pickle/tabsyn.pkl', 'rb') as f:
    train_data = pickle.load(f)
  with open('/tabsyn/synthetic/pickle/tabsyn_label.pkl', 'rb') as f:
    train_labels = pickle.load(f)
  ...
  # 테스트 데이터 로드 (실제 데이터 로드 함수로 대체 필요)
  with open('/data/posa/4/pickle/test_feature.pkl', 'rb') as f:
    test_data = pickle.load(f)
  with open('/data/posa/4/pickle/test_label.pkl', 'rb') as f:
    test_labels = pickle.load(f)
    
  with open('/data/posa/4/pickle/query_feature.pkl', 'rb') as f:
    synthetic_data = pickle.load(f)
  with open('/data/posa/4/pickle/query_label.pkl', 'rb') as f:
    synthetic_labels = pickle.load(f)
